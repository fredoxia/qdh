	...
	<div id="brandRank" data-role="page">
		<script>
			function addOrder(pbId){
				...
				function(result) {
					if (result.success) {
						var changeVal = $("#cbId option:selected").val();
			...
			<table data-role="table" id="table-column-toggle" data-mode="columntoggle" class="ui-responsive table-stroke" data-column-btn-text="挑选列..">
			     ...
			     <tbody>
					...
					<c:forEach items="${barcodeRank}" var="barcodeB">
						<tr>
						    <th style="vertical-align:middle;">${barcodeB.rank}</th>
							...
							<td style="vertical-align:middle;">${barcodeB.quantity}</td>
							<td style="vertical-align:middle;">${barcodeB.myQuantity}</td>
